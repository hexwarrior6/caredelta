...
6. Resume, Whatsapp numbers, WeChat ID  
...
* Use any resources (ChatGPT, coding copilots, open-source models, etc). Your presentation, creativity, and brief will give us strong clues about how much you care. If this problem *doesn’t* pull you in, go solve another problem. Can use synthetic data sets.   
...
*Hint:* A risk badge, a confidence label and an importance score are numbers on a screen. A build can render all three without any of them meaning anything. We will ask for each: what is it, how would we know if it were wrong, and what happens when it is? Think evaluation and abstention. Here are some decisions behind the build you should consider:

1. *Extraction or generation. Decide before you write a prompt. See Provenance \+ Trust requirements. Paraphrasing does not retain an origin, so what gets validated?*  
2. *Risk levels. Model-assigned ordinal labels inflate or drift between runs. What deterministic rules can set a floor for the model?*   
3. *Confidence. Self-reported model confidence is decoration. What does “medium” mean numerically? How does a clinician or care team see it?*  
4. *Redaction is accuracy, not just privacy. How are you ensuring this?*  
5. *Self-learning feedback loop. A few real world hazards here. Exposure bias since only surfaced items get feedback. Care team fatigue is real, so they could dismiss items under stress load so critical classes need a floor.*   
6. *Conflict detection. Scope this for allergies, medications, dosages, etc because human-human contradictions do exist between clinician and nurse and staff notes. How does your system handle this and other contradictions?*  
7. *Patient facing generation is a higher severity class.  You show a patient something hallucinated and it’s game over. Internal notes can get audited, but what’s sent to the patient needs more visible human approvals and/or rules.* 
